...
class ObsToStateBiLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.out = nn.Linear(int(hidden) * 2, int(state_dim))
    ...
    def forward(self, y: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        # lstm_out: [batch_size, seq_len, hidden_size * 2] (concatenated forward+backward at each timestep)
        # h_n: [num_layers * num_directions, batch_size, hidden_size] (final hidden states)
        # c_n: [num_layers * num_directions, batch_size, hidden_size] (final cell states)
        lstm_out, (h_n, c_n) = self.rnn(y)
