"""Transformer utilities for observation-window to full-state estimation.

This module implements a Transformer model for state estimation from observation windows.
The Transformer uses self-attention mechanisms to process observation sequences and
estimate the full state at the final time step.

References:
1) Vaswani, A., et al. (2017). Attention is all you need. 
   Advances in Neural Information Processing Systems, 30.
2) Brajard, J., et al. (2020). Combining data assimilation and machine learning to emulate 
...
class ObsToStateTransformer(nn.Module):
    """Transformer for observation-to-state mapping.
    
    This model uses self-attention to process observation windows and predict
    the full state at the final time step. The Transformer architecture allows
    the model to attend to all positions in the observation sequence.
    """
    ...
    def __init__(
        ...
        self.pos_encoding = nn.Parameter(torch.randn(1000, hidden))  # Max sequence length
        ...
        # Output projection
        self.output_proj = nn.Linear(hidden, state_dim)
    ...
    def forward(self, y: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        # Input projection
        x = self.input_proj(y)  # [batch_size, seq_len, hidden]
        
        # Add positional encoding
        pos_enc = self.pos_encoding[:seq_len, :].unsqueeze(0)  # [1, seq_len, hidden]
        x = x + pos_enc
        ...
        x = self.transformer(x, mask=mask)  # [batch_size, seq_len, hidden]
        
        # Use the last time step for prediction
        x = x[:, -1, :]  # [batch_size, hidden]
        
        # Output projection
        return self.output_proj(x)  # [batch_size, state_dim]


def train_obs_to_state_transformer(
    Y: np.ndarray,
    X_true: np.ndarray,
    *,
    window: int,
